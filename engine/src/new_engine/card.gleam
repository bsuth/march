...
import new_engine/face.{type Face}
import new_engine/suit.{type Suit}
import new_engine/trait.{type Trait}
import yuzu
...
pub fn classic_deck(color: Color) {
  [
...
pub fn classic_deal(color: Color, init_hand_size: Int) {
  classic_deck(color)
  |> list.shuffle()
  |> list.split(init_hand_size)
}

pub fn standard_deck(color: Color) {
  [
...
pub fn standard_deck_jack(suit: Suit, color: Color) {
  Card(face: face.Jack, suit:, color:, traits: standard_jack_traits)
}

pub fn standard_deck_queen(suit: Suit, color: Color) {
  Card(face: face.Queen, suit:, color:, traits: standard_queen_traits)
}

pub fn standard_deck_king(suit: Suit, color: Color) {
  Card(face: face.King, suit:, color:, traits: standard_king_traits)
}

pub fn standard_deck_ace(suit: Suit, color: Color) {
  Card(face: face.Ace, suit:, color:, traits: standard_ace_traits)
}

pub fn standard_deal(color: Color, init_hand_size: Int) {
  standard_deck(color)
  |> list.shuffle()
  |> list.split(init_hand_size)
}
